...
class ViewController: UIViewController , UITextViewDelegate {
    ...
    func setTxtView(_ str:String) {
        ...
        var strTemp = str
        for _ in arrAddedUser {
            let strTag = strTemp.slice(from: tagPrefix, to: tagPostfix)!
            for range in strTemp.ranges(of: "\(tagPrefix)\(strTag)\(tagPostfix)") {
                let rng = range.lowerBound ..< "\(strTemp)\(strTemp)".index(range.upperBound, offsetBy: -(tagPrefix.count + tagPostfix.count))
    ...
    @objc private final func tapOnTextView(_ recognizer: UITapGestureRecognizer){
        ...
        for i in 0 ..< arrRange.count {
            if arrRange[i].lowerBound.encodedOffset <= charIndex && arrRange[i].upperBound.encodedOffset > charIndex {
                print("name:-\(arrAddedUser[i])")
                lblTagName.text = arrAddedUser[i]
            }
        }
    }
            ...
            for rang in newText.ranges(of: "@") {
                if (rang.lowerBound.encodedOffset < range.upperBound) {
                    func searchRang() {
                        if (text.count == 0) {
                            let i = -range.length
                            rangSearch = "\(newText)\(newText)".index(rang.upperBound, offsetBy:  i) ..< "\(newText)\(newText)".index(rang.lowerBound, offsetBy:  range.upperBound + i - rang.lowerBound.encodedOffset)
                        } else {
                            let i = text.count
        ...
        for i in 0 ..< arrRange.count {
            ...
            if (text.count != 0) {
                ...
                if (arrRange[i].lowerBound.encodedOffset >= range.location +  range.length) {
                    arrRange[i] = "\(newString)\(newString)".index(arrRange[i].lowerBound, offsetBy: text.count - range.length) ..< "\(newString)\(newString)".index(arrRange[i].upperBound, offsetBy:text.count - range.length)
                }
                //                }
            } else {
...
//        self.txtMain.isEditable = !self.txtMain.isEditable
//        self.txtMain.isSelectable = !self.txtMain.isSelectable
        self.txtMain.removeGestureRecognizer(tapGesture)
        if (sender.isOn) {
            self.txtMain.addGestureRecognizer(tapGesture)
            self.txtMain.isEditable = false
            self.txtMain.isSelectable = false
        } else {
            self.txtMain.isEditable = true
            self.txtMain.isSelectable = true
        }
    }
}
